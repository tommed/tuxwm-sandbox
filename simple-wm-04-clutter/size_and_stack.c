...
void handle_configure(Display *d, Window w, int screen, StateMachine *state)
{
	...
	changes.x = 0; // TODO: take top panel into consideration
	changes.y = 0;
	changes.width = DisplayWidth(d, screen);
	changes.height = DisplayHeight(d, screen);
	//unsigned int changes_mask = CWX|CWY|CWWidth|CWHeight;
	...
	//XConfigureWindow(d, w, changes_mask, &changes);
